...
public class ChangeCommissionedTransactionTest {
	...
	public void changeCommissionedTransactionTest() {
		int employeeId = 3;
		AddEmployeeTransaction addEmployeeTransaction = new AddHourlyEmployeeTransaction(employeeId, "Lance", "Home",
				24.3);
		addEmployeeTransaction.execute();

		ChangeCommissionedTransaction changeCommissionedTransaction = new ChangeCommissionedTransaction(employeeId,
				20.0, 200);
		changeCommissionedTransaction.execute();

		Employee employee = database.getInstance().getEmployee(employeeId);
		assertThat(employee.getPaymentClassification(), is(instanceOf(CommissionedClassification.class)));
